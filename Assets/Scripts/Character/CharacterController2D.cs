...
public class CharacterController2D : MonoBehaviour
{
    ...
    [FoldoutGroup("Movement Variables", nameof(m_FacingRight), nameof(m_AirControl), nameof(m_MovementSmoothing), nameof(m_FacingRight), nameof(runSpeed), nameof(airRunSpeed))]
    ...
    [SerializeField, HideInInspector] public bool m_FacingRight = true;
    ...
    private void Awake()
    {
        ...
        camFollowDirection = m_FacingRight;
    }
            ...
            if (isGrounded && !isJumping)
            {
                ...
                if (isRunning)
                {
                    if (anim.GetCurrentAnimatorStateInfo(0).IsName("attack_run_" + playerAttack.attackIndex)
                          && playerAttack.isAttackingLeft != InputPlayer.instance.leftPointLeft)
                    {
        ...
        if (ray.collider != null)
        {
            ...
            if (!wasGrounded)
            {
                    ...
                    anim.GetCurrentAnimatorStateInfo(0).IsName("attack_fall_" + playerAttack.attackIndex)))
                {
                    ...
                    if (isRunning)
                    {
                        if (playerAttack.isAttackingLeft != InputPlayer.instance.leftPointLeft)
                        {
                            if (playerAttack.attackIndex == 1)
                            {
                                if (duration < (35f / 71f))
    ...
    public void Flip(bool ignoreCamFollowFlip = false)
    {
        ...
        if (!ignoreCamFollowFlip && camFollowDirection != m_FacingRight)
        {
