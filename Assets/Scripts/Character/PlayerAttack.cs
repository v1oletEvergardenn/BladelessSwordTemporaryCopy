...
public class PlayerAttack : MonoBehaviour
{
    ...
    public void Attack(bool attackLeft)
    {
        if (canAttack && !controller.isFloating && attackTimer >= attackGap)
        {
            ...
            if (controller.m_FacingRight == attackLeft) { controller.Flip(true); backAttack = true; }
    ...
    public void CheckCounterAttack()
    {
        ...
        {
            if (controller.m_FacingRight && closeTarget_proj.GetPivot().x <= transform.position.x) { return; }
            if (!controller.m_FacingRight && closeTarget_proj.GetPivot().x >= transform.position.x) { return; }
            ...
            {
                if (controller.m_FacingRight && closeTarget_obj.GetHitPos().x <= transform.position.x) { return; }
                if (!controller.m_FacingRight && closeTarget_obj.GetHitPos().x >= transform.position.x) { return; }
    ...
    public void CheckBoomerang()
    {
        ...
        else if (isBoomeranging && !_boomerang.isQTE)
        {
            if (_boomerang.stickedInToWall)
            {
                ...
                if (_boomerang.facingRight == controller.m_FacingRight)
                {
                    controller.Flip();
                }
            }
    ...
    public void LaunchBoomerang()
    {
        ...
        _boomerang.facingRight = controller.m_FacingRight;
    }
